...
.predcheck_get_eval_data <- function(data = NULL, traindata = NULL, testdata = NULL) {
  eval_data <- if (!is.null(testdata)) testdata else traindata
  if (is.null(eval_data)) eval_data <- data
  if (is.null(eval_data)) stop("predcheck: evaluation data must be provided.", call. = FALSE)
  as.data.frame(eval_data)
}
