...
#'   annotating outliers.
#' @param outlier.label.xpad Numeric padding added to the right side of the
#'   x-axis limit when outlier labels are drawn. This helps prevent labels for
#'   extreme right-side observations from being clipped.
...
qqnorm.zresid <- function(y, zcov = NULL, info = NULL, irep = 1, diagnosis.test = "SW",
  ...
  # ---- input checks ----
  outlier.label.xpad <- suppressWarnings(as.numeric(outlier.label.xpad)[1])
  ...
  for (col_id in irep) {
    ...
    xlim_default <- range(x_values, finite = TRUE)
    if (!all(is.finite(xlim_default))) {
      xlim_default <- c(-3, 3)
    }
    xlim_default[2] <- xlim_default[2] + outlier.label.xpad
    
    default_plot_args <- list(x = x_values, y = y_plot,
                              main = main.title, xlab = xlab, ylab = ylab,
                              xlim = xlim_default,pch = 1)
    user_args <- list(...)
