...
#' @param traindata Training data. Used when \code{testdata} is \code{NULL}.
#' @param testdata Optional held-out data. If supplied, the check is evaluated
#'   on this dataset.
#' @param data Backward-compatible alias for \code{traindata}.
...
predcheck_zresid <- function(fit,
  ...
  eval_data <- .predcheck_get_eval_data(
    data = data,
    traindata = traindata,
    testdata = testdata
  )
  ...
  out <- list(
    mode = if (is.null(testdata)) "PPC" else "HPC",
...
#' @param fit A fitted model object.
#' @param traindata Training data. Used when \code{testdata} is \code{NULL}.
#' @param testdata Optional held-out data.
#' @param data Backward-compatible alias for \code{traindata}.
...
.predictive_check_core <- function(fit,
  ...
  if (!is.null(predcheck_pointpred)) {
    warning(
      "`predcheck_pointpred` is deprecated and ignored. ",
      "Use `log_pointpred`, `point_Zcov`, and `postpred_simulate` bridges instead.",
      call. = FALSE
    )
  }
  ...
  eval_train <- if (!is.null(traindata)) traindata else data
  eval_test <- if (identical(mode, "hpc")) testdata else NULL

  z_rpp <- predcheck_zresid(
    fit = fit,
    traindata = eval_train,
    testdata = eval_test,
    test = test,
    log_pointpred = log_pointpred,
