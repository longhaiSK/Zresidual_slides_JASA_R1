...
This vignette demonstrates how to use the `Zresidual` package to compute component-wise Z-residuals for diagnosing Bayesian hurdle models. Based on the output from the `brms` package in R, Z-residuals can be calculated separately for the zero, count, and overall hurdle components to reveal potential model misspecifications. 
...
if (force_rerun || !file.exists(gif_qq_path_c)) {
  gifski::save_gif(
    expr = {
      for (i in 1:10) {
        par(mfrow = c(1, 3))
        qqnorm(zres_c_zero,  info = zcov_c_zero,   irep = i, main = "Zero Component")
        qqnorm(zres_c_count, info = zcov_c_count,  irep = i, main = "Count Component")
        qqnorm(zres_c_hurdle,info = zcov_c_hurdle, irep = i, main = "Hurdle Overall")
      }
    },
...
gif_scatter_path_c <- file.path(model_dir, "hnb_scatterplot_anim.gif")
...
plot(zres_c_hurdle,  info = zcov_c_hurdle,
     main = "Hurdle Overall", irep = i,
     xlab = "Observation Index", 
     ylab = "Z-residuals")
abline(h = 0, col = "red", lty = 2, lwd = 2)
}
    },
    gif_file = gif_scatter_path_c, width = 900, height = 320, res = 96, delay = 0.8
  )
}
...
if (force_rerun || !file.exists(gif_qq_path_w)) {
  gifski::save_gif(
    expr = {
      for (i in 1:10) {
        par(mfrow = c(1, 3))
        qqnorm(zres_w_zero,  info = zcov_w_zero,   irep = i, main = "Zero Component")
        qqnorm(zres_w_count, info = zcov_w_count,  irep = i, main = "Count Component")
        qqnorm(zres_w_hurdle,info = zcov_w_hurdle, irep = i, main = "Hurdle Overall")
      }
    },
